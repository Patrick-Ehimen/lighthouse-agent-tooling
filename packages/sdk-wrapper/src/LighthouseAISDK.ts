...
Maximum file size may be exceeded. Try uploading a smaller file.`);
  ...
  /**
   * Test network connectivity to Lighthouse services
   *
   * @returns Promise resolving to connectivity status and diagnostics
   */
    ...
    try {
      ...
      if (!apiKey) {
        return {
          ...
          diagnostics: ["Configure your Lighthouse API key in settings"],
        };
      }

      diagnostics.push("✓ API key configured");

      // Try to get uploads list (lightweight operation)
      const response = await this.withTimeout(
        lighthouse.getUploads(apiKey),
        10000, // 10 second timeout for connectivity test
      );

      const latency = Date.now() - startTime;
      diagnostics.push(`✓ Connected to Lighthouse API (${latency}ms)`);

      if (response && response.data) {
        const fileCount = response.data.fileList?.length || 0;
        diagnostics.push(`✓ Account accessible (${fileCount} files found)`);
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      diagnostics.push(`✗ Connection failed: ${errorMessage}`);

      // Add specific diagnostics based on error type
      if (errorMessage.includes("ETIMEDOUT") || errorMessage.includes("timeout")) {
        diagnostics.push("→ Network timeout - check internet connection");
        diagnostics.push("→ Try disabling VPN if you're using one");
        diagnostics.push("→ Check firewall settings (port 443)");
      } else if (errorMessage.includes("ENOTFOUND")) {
        diagnostics.push("→ DNS resolution failed");
        diagnostics.push("→ Check your DNS settings");
        diagnostics.push("→ Try using a different network");
      } else if (errorMessage.includes("401") || errorMessage.includes("unauthorized")) {
        diagnostics.push("→ Invalid API key or insufficient permissions");
        diagnostics.push("→ Verify your API key in Lighthouse dashboard");
      }
